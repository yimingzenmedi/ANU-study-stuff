...
checkpoint = torch.load('ckpt\\LTEVSFSMBI\\centerEsti\\centerEsti_model_best.pth')
model1.load_state_dict(checkpoint['state_dict_G'])
# checkpoint = torch.load('pretrained_models/F35_N8.pth')
# checkpoint = torch.load('ckpt\\LTEVSFSMBI\\F35_N8\\F35_N8_model_F35_epoch_300.pth')
checkpoint = torch.load('ckpt\\LTEVSFSMBI\\F35_N8\\F35_N8_model_best.pth')
model2.load_state_dict(checkpoint['state_dict_G'])
# checkpoint = torch.load('ckpt\\LTEVSFSMBI\\F26_N9\\F26_N9_model_epoch_50.pth')
checkpoint = torch.load('ckpt\\LTEVSFSMBI\\F26_N9\\F26_N9_model_best.pth')
# checkpoint = torch.load('pretrained_models/F26_N9_from_F35_N8.pth')
model3.load_state_dict(checkpoint['state_dict_G'])
# checkpoint = torch.load('ckpt\\LTEVSFSMBI\\F17_N9\\F17_N9_model_epoch_50.pth')
checkpoint = torch.load('ckpt\\LTEVSFSMBI\\F17_N9\\F17_N9_model_best.pth')
