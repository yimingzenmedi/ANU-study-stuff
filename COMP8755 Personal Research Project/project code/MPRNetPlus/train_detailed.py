...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
        ...
        for i, data in enumerate(tqdm(train_loader), 0):
            ...
            loss_char = criterion_char(restored_detailed1, target1)
            loss_edge = criterion_edge(restored_detailed1, target1)
            ...
            loss_char += criterion_char(restored_detailed7, target7)
            loss_edge += criterion_edge(restored_detailed7, target7)

            loss = (loss_char + (0.05 * loss_edge)) / 7
            print(f"\nrestored1: %.6f, restored2: %.6f, restored3: %.6f, restored4: %.6f" %
                  (restored1.mean(), restored2.mean(), restored3.mean(), restored4.mean()))
            print(f"target1:   %.6f, target2:   %.6f, target3:   %.6f, target4:   %.6f" %
        ...
        if epoch % opt.TRAINING.VAL_AFTER_EVERY == 0:
            ...
            torch.save({'epoch': epoch,
                        'state_dict': model_detailed.state_dict(),
                        'optimizer': optimizer.state_dict()
                        }, os.path.join(model_dir1, f"detailed_model_epoch_{epoch}.pth"))
