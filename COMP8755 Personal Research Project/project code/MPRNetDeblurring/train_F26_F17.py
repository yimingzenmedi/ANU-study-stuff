...
if __name__ == '__main__':
    ...
    result_dir = os.path.join(opt.TRAINING.SAVE_DIR, f"{mode}", 'results', session)
    model_dir = os.path.join(opt.TRAINING.SAVE_DIR, f"{mode}", 'models', session)
    ...
    if opt.TRAINING.RESUME:
        path_chk_rest1 = utils.get_last_path(model_dir, f'{mode}_model_{index1}_latest.pth')
        utils.load_checkpoint(model_restoration1, path_chk_rest1)
        utils.load_optim(optimizer1, path_chk_rest1)

        path_chk_rest2 = utils.get_last_path(model_dir, f'{mode}_model_{index2}_latest.pth')
        utils.load_checkpoint(model_restoration2, path_chk_rest2)
        utils.load_optim(optimizer2, path_chk_rest2)

        path_chk_rest6 = utils.get_last_path(model_dir, f'{mode}_model_{index6}_latest.pth')
        utils.load_checkpoint(model_restoration6, path_chk_rest6)
        utils.load_optim(optimizer6, path_chk_rest6)

        path_chk_rest7 = utils.get_last_path(model_dir, f'{mode}_model_{index7}_latest.pth')
        ...
        print('------------------------------------------------------------------------------')
        print(f"==> Resuming Training with learning rate - new_lr2: {new_lr1}, new_lr2: {new_lr2}, new_lr6: {new_lr6}, new_lr7: {new_lr7}")
        print('------------------------------------------------------------------------------')
    ...
    criterion = LTEVSFSMBILosses.F17_N9Loss_F26_N9Loss()

    ######### DataLoaders ###########
    train_dataset = get_training_data(train_dir, {'patch_size': opt.TRAINING.TRAIN_PS}, 7, [0, 1, 5, 6])
    ...
    for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
        ...
        if epoch % opt.TRAINING.VAL_AFTER_EVERY == 0:
            ...
            for ii, data_val in enumerate((val_loader), 0):
                ...
                # print(target1.shape)
                # print(target2.shape)
                for res1, res7, res2, res6, tar1, tar7, tar2, tar6 in zip(restored1, restored7, restored2, restored6, target1, target7, target2, target6):
            ...
            if (psnr_val_rgb1 - best_psnr[0]) + (psnr_val_rgb2 - best_psnr[1]) \
                ...
                torch.save({'epoch': epoch,
                            ...
                            }, os.path.join(model_dir, f'{mode}_model_{index1}_best.pth'))
                torch.save({'epoch': epoch,
                            'state_dict': model_restoration2.state_dict(),
                            'optimizer': optimizer2.state_dict()
                            }, os.path.join(model_dir, f'{mode}_model_{index2}_best.pth'))
                torch.save({'epoch': epoch,
                            'state_dict': model_restoration6.state_dict(),
                            'optimizer': optimizer6.state_dict()
                            }, os.path.join(model_dir, f'{mode}_model_{index6}_best.pth'))
                torch.save({'epoch': epoch,
                            'state_dict': model_restoration7.state_dict(),
                            'optimizer': optimizer7.state_dict()
                            }, os.path.join(model_dir, f'{mode}_model_{index7}_best.pth'))
            ...
            torch.save({'epoch': epoch,
                        'state_dict': model_restoration1.state_dict(),
                        'optimizer': optimizer1.state_dict()
                        }, os.path.join(model_dir, f"{mode}_model_{index1}_epoch_{epoch}.pth"))
            torch.save({'epoch': epoch,
                        'state_dict': model_restoration2.state_dict(),
                        'optimizer': optimizer2.state_dict()
                        }, os.path.join(model_dir, f"{mode}_model_{index2}_epoch_{epoch}.pth"))
            torch.save({'epoch': epoch,
                        'state_dict': model_restoration6.state_dict(),
                        'optimizer': optimizer6.state_dict()
                        }, os.path.join(model_dir, f"{mode}_model_{index6}_epoch_{epoch}.pth"))
            torch.save({'epoch': epoch,
                        'state_dict': model_restoration7.state_dict(),
                        'optimizer': optimizer7.state_dict()
                        }, os.path.join(model_dir, f"{mode}_model_{index7}_epoch_{epoch}.pth"))
        ...
        torch.save({'epoch': epoch,
                    'state_dict': model_restoration1.state_dict(),
                    'optimizer': optimizer1.state_dict()
                    }, os.path.join(model_dir, f'{mode}_model_{index1}_latest.pth'))
        torch.save({'epoch': epoch,
                    'state_dict': model_restoration2.state_dict(),
                    'optimizer': optimizer2.state_dict()
                    }, os.path.join(model_dir, f'{mode}_model_{index2}_latest.pth'))
        torch.save({'epoch': epoch,
                    'state_dict': model_restoration6.state_dict(),
                    'optimizer': optimizer6.state_dict()
                    }, os.path.join(model_dir, f'{mode}_model_{index6}_latest.pth'))
        torch.save({'epoch': epoch,
                    'state_dict': model_restoration7.state_dict(),
                    'optimizer': optimizer7.state_dict()
                    }, os.path.join(model_dir, f'{mode}_model_{index7}_latest.pth'))
