...
class Generic_train_test():
    ...
    def test_individual(self, _input, dir_results, i, im_name=""):
        self.model.set_input(_input)

        start_time = time.time()
        im_pred = self.model.eval()
        print(f">> im_pred: {im_pred.shape}")
        ...
        if self.opts.save_images:
            ...
            if im_name != "":
                im_name += "_"
            # io.imsave(os.path.join(dir_results, f'{im_name}%04d_blur.png' % i), np_im_in)
            io.imsave(os.path.join(dir_results, f'{im_name}%04d_pred.png' % i), np_im_pred)
            if self.opts.compute_metrics:
                np_im_gt = white_balance(
                    self.model.im_target.detach().cpu().numpy().transpose(0, 2, 3, 1)[0].clip(0., 1.))
