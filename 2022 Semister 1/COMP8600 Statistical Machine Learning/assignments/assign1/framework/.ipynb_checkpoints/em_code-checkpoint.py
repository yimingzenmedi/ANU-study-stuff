...
def make_GMM_data():
    ...
    cov_gt = np.stack([(np.array([0.4])), (np.array([0.6])), (np.array([0.2]))], axis=0)
    gaussian_1 = np.random.randn(200) * cov_gt[0] + mu_gt[0]
    gaussian_2 = np.random.randn(160) * cov_gt[1] + mu_gt[1]
    gaussian_3 = np.random.randn(400) * cov_gt[2] + mu_gt[2]
    data = np.concatenate([gaussian_1, gaussian_2, gaussian_3], axis=0).squeeze()
    return pi_gt, mu_gt, cov_gt, gaussian_1, gaussian_2, gaussian_3, data, Nk_gt
